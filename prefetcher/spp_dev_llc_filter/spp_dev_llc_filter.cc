...
uint32_t spp_dev_llc_filter::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
...
uint32_t spp_dev_llc_filter::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
bool spp_dev_llc_filter::PREFETCH_FILTER::check(champsim::address check_addr, FILTER_REQUEST filter_request)
{
  ...
  case spp_dev_llc_filter::SPP_LLC_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      ...
      auto llc_filter_entry = llc_filter.check_hit(llc_entry{champsim::block_number{check_addr},0});
      if(llc_filter_entry.has_value()) {
        if(_parent->intern_->current_cycle() - llc_filter_entry->first_accessed > LLC_FILTER_TIMEOUT)
          llc_filter.invalidate(llc_filter_entry.value());
        else
          return false;
      }
      else {
        llc_filter.fill(llc_entry{champsim::block_number{check_addr},_parent->intern_->current_cycle()});
      }
